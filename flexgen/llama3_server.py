...
from flexgen.qwen2_config import get_qwen_config
...
from flexgen.qwen_backend import QwenTorchDevice
...
from flexgen.flex_qwen2 import QwenLM
...
if "llama" in MODEL:
    model_config = get_llama_config(MODEL, pad_token_id=tokenizer.eos_token_id)
    gpu = Llama3TorchDevice("cuda:0", rope_config=model_config.rope_config)
...
print("init weight...")
if "llama" in MODEL:
    model = LlamaLM(model_config, env, PATH, policy, PREFILL_BATCH_SIZE)
elif "qwen" in MODEL:
    model = QwenLM(model_config, env, PATH, policy, PREFILL_BATCH_SIZE)
...
def run_flexgen(chat, max_tokens):
    ...
    return result.replace("<|start_header_id|>assistant<|end_header_id|>\n\n","")\
